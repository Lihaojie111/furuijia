...
class Index extends  Controller{
  ...
  public function getindex()   
  {
    ...
    $ynopsis=Db::table('ynopsis')->find();
        //资讯中心
    $news=Db::table('new')->where('state','0')->limit(5)->order('id desc')->select();
      //加载前台首页
    return $this->fetch("Mobile/index",['map'=>$map,'cate'=>$cate,'promotion'=>$promotion,'room'=>$room,'ynopsis'=>$ynopsis,'exhibition'=>$exhibition,'news'=>$news,'room2' => $room2,'map2'=>$map2]);
  }
...
public function getprofile(){
  ...
  return $this->fetch("Mobile/profile",['yonpsis'=>$yonpsis,'culture'=>$culture,'glory'=>$glory,'nav'=>$nav]);
}
...
public function getnews(){
 ...
 return $this->fetch("Mobile/news",['news'=>$news,'nav'=>$nav]);
...
public  function getnewlook(){
  ...
  return $this->fetch("mobile/news-dec",['look'=>$look,'sy'=>$sy,'xy'=>$xy]);
...
foreach ($cate as $key => $value) {
 ...
 return $this->fetch("Mobile/team",['data'=>$data,'cate'=>$cate,'nav'=>$nav]);
...
public function  getexhibition(){
  ...
  return $this->fetch("Mobile/decorate-eigth",['exhibition'=>$exhibition,'cate'=>$cate,'nav'=>$nav]);
...
public function  getdetails(){
  ...
  return $this->fetch("Mobile/decorate-nine",["exhi"=>$exhi,"sy"=>$sy,"xy"=>$xy,'nav'=>$nav]);
...
public function  getdostyle(){
        ...
        // return $this->fetch("Mobile/decorate-eigth",['exhibition'=>$exhi,'cate'=>$cate]);
...
public function  getzhaoshang(){
  ...
  return $this->fetch("Mobile/zhaoshang",['nav'=>$nav]);
...
public function  postzhaoshang(){
  ...
  if(Db::table('zhaoshang')->insert($request)){
    echo '<script>alert("提交成功");window.location.href="/Mobile/zhaoshang"</script>';
  }else{

    echo '<script>alert("提交失败");window.location.href="/Mobile/zhaoshang"</script>';
...
public function getdescribes(){
  ...
  return $this->fetch("Mobile/opiys",["room"=>$room,'room2' => $room2,'nav'=>$nav]);
